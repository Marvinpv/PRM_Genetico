...
bool LeerCabecera (ifstream& f, unsigned& fils, unsigned& cols){
    int maxvalor;
    string linea;
...
vector<unsigned char> LeerImagenPGM (const char *nombre, unsigned& fils, unsigned& cols){
    ...
    if (f && LeerTipo(f) == TipoImagen::IMG_PGM){
        if (LeerCabecera (f, fils, cols)){
    ...
    vector<unsigned char> ret(res, res + fils*cols);
    return ret;
}
...
bool EscribirImagenPGM (const char *nombre, const unsigned char *datos, 
                        const unsigned fils, const unsigned cols){
  ofstream f(nombre);
  bool res= true;
